...
async def google_callback(
    ...
    try:
        ...
        user = await user_service.get_user_by_google_id(validated_user_info['google_id'])
        
        if not user:
            # Try to find by email
            user = await user_service.get_user_by_email(validated_user_info['email'])
            ...
            if user:
                ...
                user = await user_service.create_oauth_user(
                    ...
                    picture=validated_user_info['picture'],
                    oauth_provider='google',
                    email_verified=validated_user_info['verified_email']
                )
