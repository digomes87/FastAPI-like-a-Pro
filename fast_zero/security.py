    ...
    async def check_login_security(
        ...
        if await self.rate_limiter.is_rate_limited(client_ip):
            raise HTTPException(
                ...
                headers={
                    'Retry-After': str(settings.RATE_LIMIT_WINDOW_MINUTES * 60)
                },
            )

        # Check account lockout
        if await self.account_lockout.is_locked_out(username):
            lockout_info = await self.account_lockout.get_lockout_info(
                username
            )
            if lockout_info:
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        f'{lockout_info["remaining_minutes"]} minutes.'
                    ),
                    headers={
                        'Retry-After': str(
                            lockout_info['remaining_minutes'] * 60
                        )
                    },
                )

    async def record_login_attempt(
        self, request: Request, username: str, success: bool
    ) -> None:
